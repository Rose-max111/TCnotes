...
\begin{equation}
    ...
    \Delta \sim e^{-W}.
\end{equation}
Consequently, $t_{th}$ is related to $\epsilon$ by $\left( 1-\Delta \right) ^ {t_{th}} = \epsilon$, which gives $t_{th} \sim \ln\left(\frac{M}{W}\right)e^{W}$, and the total sweep times scale as:
\begin{equation}\label{eq:totaltime}
t_{\rm total} \sim \frac{M}{W}\ln\left(\frac{M}{W}\right)e^W.
\end{equation}
...
\begin{equation}
    e^{-\frac{\Delta E}{T_{\rm max}}\Lambda^{cW'}} \sim \epsilon
\end{equation}
where $c$ is an empirical constant and can be determined during the experiment. 
\jinguo{How about this?}
\ym{But I still don't think the constant $c$ is reasonable in this version. Anyway it is a better 
description for $W'$.}

We can further derive $e^{W'}$ as function of $\Lambda$:
\begin{equation}
    e^{\ln(\Lambda)cW'}\sim \ln(\frac{1}{\epsilon})\rightarrow e^{W'}\sim \ln(\frac{1}{\epsilon})^{\frac{1}{c\ln(\Lambda)}}
\end{equation}

Combining with \Cref{eq:totaltime} and $\epsilon \sim \frac{W'}{M}$, the total annealing time can be estimated as
\begin{equation}
    \label{eq:total-annealing-time}
    t_{\rm total} \sim \frac{M}{W'}\left(\ln\left(\frac{M}{W'}\right)\right)^{1+ \frac{1}{c\ln(\Lambda)}}
\end{equation}
