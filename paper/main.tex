...
\begin{abstract}
    Utilizing thermodynamics to perform computation can be more energy efficient than conventional computers.
    However, a theoretical model of thermodynamics driven computation in solid state physical system is still unavailable.
    In this paper, we propose a crystalline structure that enables use to perform thermodynamics driven computation by editing its surface.
    % It is based on a universal elementary CA, and is translational invariant in all spacial dimensions.
    Its translational invariance enables us to systematically study the scaling behavior of the thermalization process.
...
\jinguo{
    TODO: list some questions for discussion with Monte Carlo expert.
}

\ym{
    \begin{enumerate}
        \item There are many discussion about the relation between the sampling time and 
        the ``quasi-equilibrilium-distribution''. However, here we don't need it to be so close
        to the equilibrilium-distribution. How to quantify the relation between sampling time and error rate?

        \item We thought the error rate should decrease expoenentially with the sampling time in conventional SA
        however, numerical results does not support our hypothesis. Why?
    \end{enumerate}
}
...
% \end{enumerate}
% }
% \jinguo{We either discuss general TC, and then gadget design, or use the spin glass model in our study.}

Improving the energy efficiency of computation is a major challenge in the development of future computing systems.
One of the attempts to improve the energy efficiency is building a thermodynamic computer (TC)~\cite{conte2019thermodynamic, hylton2021vision}.
...
It utilizes the tendency of a physical system to evolve towards the state with the lowest free energy.
By manipulating the minimum free energy state of the system, the computation can be performed probabilistically and energy efficiently.
It can be achieved by changing the distribution of the particles in the system, such as in the case of DNA machines~\cite{Feynman2018, tanaka2005design}, or lowering the temperature of the system, such as in the case of Ising machines~\cite{boixo2013experimental, gu2012encoding, lucas2014ising, mohseni2022ising, fabre2014optical}.
...
In this work, we focus on the energy based models.
They are one of the most powerful models for computation as the ground states of which can encode the solution of any problems in NP~\cite{Moore2011, gu2012encoding, lucas2014ising,mohseni2022ising} - the decision problems that can be verified in time polynomial to the size of the problem.
However, ground state finding in these models is generally hard due to the frustration caused by the interactions.
Although there are evidence that quantum entanglement can help improve the performance by some polynomial~\cite{boixo2014evidence,Grover1992,Pichler2018, Ebadi2022, Nguyen2023}, solving the problem in polynomial time is still intractable in any physical system~\cite{Bernstein1993}.
The hardness of thermalization an object is just like other fundamental law of thermodynamics, blocking us from building some interesting ``machines''.
This time, the interesting machine is an Ising machine that always thermalizes in polynomial time - if can be built, even theorem proving can be trivialized~\cite{Moore2011}.

Then which subset of Ising machines that can be efficiently thermalized, and can some of them used for efficient universal computation?
Intrigued by this question, we designed an energy based TC as a translational invariant Ising model. It has universal computational capability on its surface, i.e. the program can be encoded by editing its surface configuration.
With this model, we study how to efficiently thermalize the system to perform computation.
Our TC embeds the evolution of a universal elementary cellular automata (CA)~\cite{Wolfram1983} into a two mensional grid, the translational invariance in both time and space dimension enables us to systematically study the scaling behaviors the thermalization process.
We hope this study can provide some intuition for future design of TC.

In the rest of this paper, we first show the general idea of the thermal computer in \Cref{sec:Overview-of-main-results}.
The details are introduced in \Cref{sec:model-method}.
Finally, we reported numerical simulation results in \Cref{sec:numerical-result}.
%We show that even when the encoded problem is a deterministic computation, the system is not guaranteed to thermalize in time polynomial to the size of the crystal.
%However, if we use the moving temperature gradient method, the system can thermalize in time almost linear to the size of the crystal, i.e. as efficient as a normal computer.
...
\jinguo{
    \begin{enumerate}
        \item discuss the spacial encoding of the CA.
        \item discuss the thermalization of this model.
    \end{enumerate}
}
...
In cellular automata (CA), ``space'' is a regular lattice of ``cells'', each of which contains an integer from a small set.
Only cells that are close together interact in one ``time-step'' - the time evolution is given by a rule that looks at the contents of a few neighbouring cells, and decides what should change. At each step, this local rule is applied everywhere simultaneously~\cite{Margolus1984}.
Depending on the local rule, some CAs can be more interesting than others.
The most interesting category is those proven to be universal~\cite{Berlekamp2004}, meaning that they can simulate any computer program. The best-known examples in two-dimensional include Conway's~\cite{Games1970} ``Game of Life'', Margolus's reversible CA~\cite{Margolus1984} and the quantum block cellular automata~\cite{Arrighi2012}.

Interestingly, even on a one dimension lattice, we can construct universal CAs.
The simplest among which is the rule 110 elementary CA~\cite{Cook2009}.%, which is also the first proved universal elementary CA.
An elementary CA is defined on a one dimensional lattice, each site can contain state 0 or 1.
The rule of an elementary CA to determine the state of the cell in next generation depends only on the current state of the cell and its two immediate neighbors,
hence the number of possible elementary CA rules is equal to the number of 3-input Boolean functions, which is $2^{2^3} = 256$.
The Boolean function for rule 110 is shown in the following table:
...
In the proposed TC, we embed a $d$-dimensional CA into a $(d+1)$-dimensional energy model, where the extra dimension is used to encode the time-evolution of the CA.
The TC has a translational invariant structure, as shown in \Cref{Surface-programmable-thermodynamic computer}.
The repeating structures are associated with a same energy model representing the transition rule of the CA.
%The ground state of the energy model is associated with the state transition encoded in the transition rule.
In the following, we will show how to encode the transition rule of the CA into the Ising model:
\begin{equation}\label{eq:ising}
E(\vs) = \sum_{i < j} J_{ij}s_us_v + \sum_{i=1}^{n}h_i s_i,
\end{equation}
where $n$ is the number of spins, $h_i$ is onsite energy, $J_{ij}$ is interaction energy and
$s_i \in \{-1, 1\}$ is local spin. $s_i=1$ means spin-up that corresponds to the state $1$ of the CA and $s_i=-1$ means spin-down that corresponds to the state $0$ of the CA.
...
%Shape: Ellipse [id:dp3980815479811872] 
\draw  [fill={rgb, 255:red, 255; green, 255; blue, 255 }  ,fill opacity=1 ] (493.14,156.7) .. controls (493.14,153.44) and (495.79,150.79) .. (499.05,150.79) .. controls (502.31,150.79) and (504.96,153.44) .. (504.96,156.7) .. controls (504.96,159.97) and (502.31,162.61) .. (499.05,162.61) .. controls (495.79,162.61) and (493.14,159.97) .. (493.14,156.7) -- cycle ;
%Shape: Ellipse [id:dp8208493476316323] 
\draw  [fill={rgb, 255:red, 255; green, 255; blue, 255 }  ,fill opacity=1 ] (493.14,205.29) .. controls (493.14,202.03) and (495.79,199.38) .. (499.05,199.38) .. controls (502.31,199.38) and (504.96,202.03) .. (504.96,205.29) .. controls (504.96,208.56) and (502.31,211.2) .. (499.05,211.2) .. controls (495.79,211.2) and (493.14,208.56) .. (493.14,205.29) -- cycle ;
%Shape: Circle [id:dp3173494411067954] 
\draw  [fill={rgb, 255:red, 255; green, 255; blue, 255 }  ,fill opacity=1 ] (476.94,181) .. controls (476.94,177.74) and (479.59,175.09) .. (482.85,175.09) .. controls (486.12,175.09) and (488.76,177.74) .. (488.76,181) .. controls (488.76,184.26) and (486.12,186.91) .. (482.85,186.91) .. controls (479.59,186.91) and (476.94,184.26) .. (476.94,181) -- cycle ;
%Shape: Ellipse [id:dp967421569400009] 
\draw  [fill={rgb, 255:red, 255; green, 255; blue, 255 }  ,fill opacity=1 ] (509.42,181) .. controls (509.42,177.74) and (512.07,175.09) .. (515.33,175.09) .. controls (518.6,175.09) and (521.24,177.74) .. (521.24,181) .. controls (521.24,184.26) and (518.6,186.91) .. (515.33,186.91) .. controls (512.07,186.91) and (509.42,184.26) .. (509.42,181) -- cycle ;
%Shape: Ellipse [id:dp938870324276512] 
\draw  [fill={rgb, 255:red, 255; green, 255; blue, 255 }  ,fill opacity=1 ] (537.4,180.87) .. controls (537.4,177.6) and (540.04,174.96) .. (543.31,174.96) .. controls (546.57,174.96) and (549.22,177.6) .. (549.22,180.87) .. controls (549.22,184.13) and (546.57,186.78) .. (543.31,186.78) .. controls (540.04,186.78) and (537.4,184.13) .. (537.4,180.87) -- cycle ;
%Rounded Rect [id:dp40904235215635065] 
\draw  [fill={rgb, 255:red, 235; green, 83; blue, 112 }  ,fill opacity=1 ] (472.92,157.94) .. controls (472.92,149.47) and (479.78,142.6) .. (488.26,142.6) -- (537.95,142.6) .. controls (546.42,142.6) and (553.29,149.47) .. (553.29,157.94) -- (553.29,203.95) .. controls (553.29,212.43) and (546.42,219.29) .. (537.95,219.29) -- (488.26,219.29) .. controls (479.78,219.29) and (472.92,212.43) .. (472.92,203.95) -- cycle ;
%Straight Lines [id:da6938220604961829] 
\draw    (493.14,156.7) -- (465.3,156.7) ;
%Straight Lines [id:da05667244881651223] 
\draw    (476.94,180.82) -- (465.3,180.87) ;
%Straight Lines [id:da7336130308225484] 
\draw    (493.14,205.29) -- (465.3,205.29) ;
%Straight Lines [id:da9632416883191204] 
\draw    (482.85,181) -- (499.05,156.7) ;
%Straight Lines [id:da8062551247029028] 
\draw    (482.85,181) -- (499.05,205.29) ;
%Straight Lines [id:da6815111207533529] 
\draw    (499.05,156.7) -- (515.33,181) ;
%Straight Lines [id:da2583696463055196] 
\draw    (499.05,205.29) -- (515.33,181) ;
%Straight Lines [id:da5112788448231447] 
\draw    (482.85,181) -- (515.33,181) ;
%Straight Lines [id:da21092112099851623] 
\draw    (499.05,205.29) -- (499.05,178.37) -- (499.05,156.7) ;
%Curve Lines [id:da6381556136782309] 
\draw    (499.05,205.29) .. controls (516.94,205.22) and (533.75,202.59) .. (543.31,180.87) ;
%Curve Lines [id:da40382455887046964] 
\draw    (499.05,156.7) .. controls (520.61,156.36) and (536.9,162.14) .. (543.31,180.87) ;
%Curve Lines [id:da518536427411914] 
\draw    (543.31,180.87) .. controls (523.77,163.72) and (499.6,165.82) .. (482.85,181) ;
%Curve Lines [id:da5213323072929468] 
\draw    (543.31,180.87) .. controls (535.85,191.56) and (520.61,190.51) .. (515.33,181) ;
%Straight Lines [id:da26081701800713075] 
\draw    (549.22,180.87) -- (559.85,180.87) ;
%Shape: Ellipse [id:dp28592847133985866] 
\draw  [fill={rgb, 255:red, 255; green, 255; blue, 255 }  ,fill opacity=1 ] (476.94,181) .. controls (476.94,177.74) and (479.59,175.09) .. (482.85,175.09) .. controls (486.12,175.09) and (488.76,177.74) .. (488.76,181) .. controls (488.76,184.26) and (486.12,186.91) .. (482.85,186.91) .. controls (479.59,186.91) and (476.94,184.26) .. (476.94,181) -- cycle ;
%Shape: Ellipse [id:dp7005363351052425] 
\draw  [fill={rgb, 255:red, 255; green, 255; blue, 255 }  ,fill opacity=1 ] (493.14,156.7) .. controls (493.14,153.44) and (495.79,150.79) .. (499.05,150.79) .. controls (502.31,150.79) and (504.96,153.44) .. (504.96,156.7) .. controls (504.96,159.97) and (502.31,162.61) .. (499.05,162.61) .. controls (495.79,162.61) and (493.14,159.97) .. (493.14,156.7) -- cycle ;
%Shape: Ellipse [id:dp5131516311175763] 
\draw  [fill={rgb, 255:red, 255; green, 255; blue, 255 }  ,fill opacity=1 ] (493.14,205.29) .. controls (493.14,202.03) and (495.79,199.38) .. (499.05,199.38) .. controls (502.31,199.38) and (504.96,202.03) .. (504.96,205.29) .. controls (504.96,208.56) and (502.31,211.2) .. (499.05,211.2) .. controls (495.79,211.2) and (493.14,208.56) .. (493.14,205.29) -- cycle ;
%Shape: Ellipse [id:dp38829310474408074] 
\draw  [fill={rgb, 255:red, 255; green, 255; blue, 255 }  ,fill opacity=1 ] (509.42,181) .. controls (509.42,177.74) and (512.07,175.09) .. (515.33,175.09) .. controls (518.6,175.09) and (521.24,177.74) .. (521.24,181) .. controls (521.24,184.26) and (518.6,186.91) .. (515.33,186.91) .. controls (512.07,186.91) and (509.42,184.26) .. (509.42,181) -- cycle ;
%Shape: Ellipse [id:dp6003377245628321] 
\draw  [fill={rgb, 255:red, 255; green, 255; blue, 255 }  ,fill opacity=1 ] (537.4,180.87) .. controls (537.4,177.6) and (540.04,174.96) .. (543.31,174.96) .. controls (546.57,174.96) and (549.22,177.6) .. (549.22,180.87) .. controls (549.22,184.13) and (546.57,186.78) .. (543.31,186.78) .. controls (540.04,186.78) and (537.4,184.13) .. (537.4,180.87) -- cycle ;
%Straight Lines [id:da025020526203919813] 
% \draw    (305.63,73.33) -- (329.63,73.33) ;
% \draw [shift={(331.63,73.33)}, rotate = 180] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Shape: Brace [id:dp8023308836405996] 
% \draw   (283.2,286.4) .. controls (283.2,291.07) and (285.53,293.4) .. (290.2,293.4) -- (314.2,293.4) .. controls (320.87,293.4) and (324.2,295.73) .. (324.2,300.4) .. controls (324.2,295.73) and (327.53,293.4) .. (334.2,293.4)(331.2,293.4) -- (358.2,293.4) .. controls (362.87,293.4) and (365.2,291.07) .. (365.2,286.4) ;
    ...
    % Text Node
    \draw (494.80,152.01) node [anchor=north west][inner sep=0.75pt]   [align=left] {{\scriptsize {$p$}}};
    % Text Node
    \draw (478.75,176.45) node [anchor=north west][inner sep=0.75pt]   [align=left] {{\scriptsize {$q$}}};
    % Text Node
    \draw (495.30,201.60) node [anchor=north west][inner sep=0.75pt]   [align=left] {{\footnotesize {$r$}}};
    % Text Node
    \draw (539.00,174.95) node [anchor=north west][inner sep=0.75pt]   [align=left] {{\scriptsize {$q'$}}};
    % Text Node
    \draw (510.60,175.95) node [anchor=north west][inner sep=0.75pt]   [align=left] {{\scriptsize {$A$}}};
...
The logic gates are encoded in the degenerate ground states of the Ising gadgets~\cite{Aadit2022, onizawa2020design, camsari2017stochastic}.
By driving the system to the ground state, the spins naturally satisfies the logic expression.
The conjunction of logic gates can be represented as the summation of the corresponding Ising Hamiltonians.
For example the logic expression $z = \neg (a \land b)$ can be encoded into the Ising gadget by combining two Hamiltonians:
\begin{equation}
    \begin{split}
    ...
    \end{split}
\end{equation}
where we add an ancilla spin $c$ to help encode the logic gate.
By driving the system to the ground state, all component Hamiltonians are minimized, and the logic expression is satisfied.

A CA can be viewed as a translational invariant logic circuit, thus can be encoded into an Ising model.
Directly synthesizing the logic circuit for the universal $110$ rule (\Cref{eq:110rule}) requires at least 3 ancilla spins, which is far from optimal.
In \Cref{sec:gadget-design}, we introduce a provable optimal method to design Ising gadgets by converting the problem to a linear programming problem~\cite{Kolman1995}.
With the new algorithm, we show that the $5$-spin gadget for the $110$ rule given below is optimal:
\begin{equation}
...
\end{blockarray},
\end{equation}
where $p, q, r$ and inputs and $q'$ correspond to the output in \Cref{Surface-programmable-thermodynamic computer}, and $A$ is the auxiliary spin.
...
By concatenating this gadget in a translational invariant manner, we construct a 2-dimensional TC that implements the $110$ rule CA that is capable of universal computing.
%This $110$ rule CA has been proved to be Turing complete~\cite{Cook2009}. Therefore, this 2-dimensional TC is capable of universal computing.
The inputs are encoded in the spins on the left surface.
To fix an input to $0$ or $1$, we raise or lower the external field $h_i$ by $\Delta_{in}>0$.
Similarly, we can also set the outputs by raising or lowering the energy of spins on the right surface.
...
We denote the computing process starting from a fixed input as the \emph{deterministic computation}, and the those starting from a fixed output as the \emph{non-deterministic computation}.

\subsection{Compute by annealing}\label{simulated-annealing-algorithm}

We consider annealing as the evolving dynamics of the system and are interested in how efficiently it can be to perform the computation.
The noticeable difference between the annealing computation and the conventional computation is that the former does not care about the ``computing direction''.
Intuitively, annealing to perform the deterministic computation is much easier than to perform the non-deterministic computation.
...
Global annealing by gradually lowering the temperature, as the Ising machine~\cite{fabre2014optical}, may still be hard for a deterministic computation, no matter how large the input detuning $\Delta_{in}$ is.
It can be rigorous proved that the annealing process remains difficult for driving the system to a ground state if the energy landscape of the optimization problem exhibits the overlap gap property~\cite{Gamarnik2021}, 
which means it cannot be solved by any local algorithms in polynomial time with respect to the system size, including both physical annealing and the simulated annealing method used in this work.
\ym{Any reference for the previous hypothesis ``...cannot be solve...''}
...
\begin{hyp}\label{hyp:overlap-gap}
...
\ym{I don't understand the following sentence. ``...set of ...'' seems weird.}
The overlap gap property describes the algorithmic intractability in random structures, which is based on the topological disconnectivity of the set of pair-wise distances of near optimal solutions.
% If the energy landscape of an optimization problem exhibits the overlap gap property, then it can not be solved by any local algorithms in polynomial time, including the simulated annealing method used in this work.
The intuition of the hypothesis comes from the energy model encoding of the identity circuit.
% Given a circuit with $n$ input bits, the ground state degeneracy of the corresponding energy model is at least $2^n$.
Consider the simplest model consisting of vertically stacked $n$ copy gadgets, where each gadgets consists of $d \sim 2^{\gamma \sqrt{n}}$ copy gates.
There are at least $2^n$ degenerate ground states in this model, and the Hamming distance between any two different ground states is at least $d$, which is exponential to $n$. Let us define the size of the problem as $d\times n$, then this distance is clearly linear to the problem size,
which means  it exhibits the overlap gap property. 
We left the rigorous proof of this hypothesis to mathematicians.

Since there is no standard approach to model the physical annealing process, we consider a theoretical model for the dynamics, which is the simulated annealing (SA)~\cite{Kirkpatrick1983} algorithm that detailed in \Cref{app:sa}.

\subsection{Dynamic annealing}\label{cooling-computation-with-moving-heat-bath}

\subsubsection{Block-wise annealing}


To overcome the overlap gap property, we propose to do block-wise annealing.
As shown in \Cref{Surface-programmable-thermodynamic computer}, we divide the lattice into $\frac{M}{W}$ blocks along the time dimension, each having width $W$.
%by introducing a heat zone with width $W$.
%It represents the region of the system that interacts with the heat bath, as shown in \Cref{Surface-programmable-thermodynamic computer}.
%Let layer $i$ be defined as the set of spins with x-coordinate $i$.
These blocks are initialized to a low enough temperature, so that the spins are freezed into a random configuration.

We anneal these blocks sequentially from left to right by heating up a block to a temperature higher than the energy scale and then cooling it down.
%We treat it as an ideal bath as it maintains a space-invariant temperature within the heat zone while keeping other regions at zero temperature.
Given a slow enough cooling rate, the spins in the block will settle into the local ground state of this block.
This sequential annealing process would not introduce frustration between blocks since there always exists a global ground state.
% The cooling rate determines the true computing speed of this model.
% It must scale with the number of computational steps $M$, otherwise, the probability of success will exponentially decrease with the number of blocks.
% How cooling rate should scale with $M$ becomes a key question in this work.

Assume the SA procedure in each block has an error probability $\epsilon$.
Repeating the procedure in $\frac{M}{W}$ blocks results in a success probability of $(1 - \epsilon)^{\frac{M}{W}}$ for finding the global ground state. 
To fix the success probability for arbitrary $M$, we must have $\epsilon \sim \frac{W}{M}$.
The annealing time $t_{th}$ required to achieve a certain error in each block is determined by the spectral gap $\Delta$ of the Markov transition matrix of the SA update on the block.
%Since the largest eigenvalue of a transition matrix is 1, the spectral gap is $\Delta = 1 - \lambda_2$, where $\lambda_2$ is the second-largest eigenvalue.
The scaling behavior of $\Delta$ and block size $W$ can be complicated, in the worst case, it can be exponential, i.e.,
\begin{equation}
    \label{eq:spectral-gap}
    \Delta \sim e^{-f(W)},
\end{equation}

[{\color{red}{**WARNING**: $(1-\Delta)^{t_{th}} = \epsilon$ doesn't match the numerical results. Please check
part ``Error rate in one block'' for details }}]

where $f(W)$ is a polynomial of $W$. Consequently, $t_{th}$ is related to $\epsilon$ by $\left( 1-\Delta \right) ^ {t_{th}} = \epsilon$, which gives $t_{th} \sim \ln\left(\frac{M}{W}\right)e^{f(W)}$, and the total sweep times scale as:
\begin{equation}\label{eq:totaltime}
t_{\rm total} \sim \frac{M}{W}\ln\left(\frac{M}{W}\right)e^{f(W)}.
\end{equation}
...
\subsubsection{Annealing by a moving heat bath}\label{estimation-of-the-computing-time}

[{\color{red}{**WARNING**: Because of the error in the previous section, the following part is also wrong}}]

%In practice, the heat bath and heat flux should establish a temperature gradient in the system.
% Suppose the heat bath only connects to a small rectangle region of the system.
Instead of block-wise annealing, we propose to do a similar sequential annealing by moving a heat bath from the left surface to the right surface at a constant speed $v$.
At time $t$, the temperature distribution $T$ is given by $T_{\rm max}\Lambda^{-|x - vt|}$, where $x$ is the computational step, $T_{\rm max}$ is the maximum temperature and $\Lambda > 1$ is a constant that determines the width of the heat zone.
...
We can define an effective heat zone with width $W'$ as the width of the raising edge of the temperature distribution, where the spins can be cooled actively.
%Defining $\Delta E_{max}$ to be the largest possible energy change when a spin is fliped.
%The effective heat zone with width $W'(\Lambda)$ should then be $[y-W'(\Lambda), y]$, where position $y$ satisfying 
%$\exp(\frac{\Delta E_{max}}{T(y)}) = 0.1$ and $y<L(t)$. 
% Since the layers in fronts of the effective heat zone can only be fliped with a $\epsilon$ probability if the local energy would increase. 
The spins outside the effective heat zone are either frozen (too cold) or flip randomly (too hot).
As a spin flips with a probability $\sim e^{-\Delta E/T}$, the cold edge of the heat zone can be defined as the position where the probability of flipping is $\epsilon$.
We have $W'$ scales with the required error probability $\epsilon$ as:
\begin{equation}
    e^{-\frac{\Delta E}{T_{\rm max}}\Lambda^{W'}} \sim \epsilon
\end{equation}
% \ym{But I still don't think the constant $c$ is reasonable in this version. Anyway it is a better 
% description for $W'$.}
% \jinguo{I changed $c$ to $1/c$}

We can further derive $e^{W'}$ as function of $\Lambda$ by taking the logarithm of both sides:
\begin{equation}
    e^{\ln(\Lambda)W'}\sim \ln(\epsilon^{-1})\rightarrow e^{W'}\sim \ln(\epsilon^{-1})^{\frac{1}{\ln(\Lambda)}}
\end{equation}

Let $f(x) = c x + b$, then $e^{f(W')} \sim \ln(\epsilon^{-1})^{\frac{c}{\ln(\Lambda)}}$.
Combining with \Cref{eq:totaltime} and the requirement of $\epsilon \sim \frac{W'}{M}$, the total annealing time can be estimated as
\begin{equation}
    \label{eq:total-annealing-time}
    t_{\rm total} \sim \frac{M}{W'}\ln\left(\frac{M}{W'}\right)^{1+ \frac{c}{\ln(\Lambda)}}
\end{equation}
The reason why we choose $f$ to be linear is due to the simplicity of the model, and it turns out to be a good approximation in the numerical simulation.
...
%Numerical result shows that computing complexity is linear to $N$.
%Thus a total sweep time for the computation should scale as $NM\log^{1 + \frac{c}{\ln(\Lambda)}}(M)$.
% \ym{Please help me to think about how to evaluate the time complexity while $N$ increases.}\jinguo{Leave it to Maddie}
The computing speed $v$ defined as operations per unit time, can be estimated as
\begin{equation}
    \label{eq:moving-speed}
    v = \frac{M}{t_{total}} \sim \ln(M)^{-(1 + \frac{c}{\ln(\Lambda)})}
\end{equation}
As the computational step $M$ increases, the computing speed $v$ decreases logarithmically.
...
Instead of studying the spin-glass model directly, we propose a toy energy model to enable large-scale numerical simulations.
%on a 2D lattice of size $N \times M$.
The energy model on a 2D lattice of size $N \times M$ is defined as:
...
Modeling temperature gradient could be achieved by using the following non-equilibrium SA acceptance probability:
\begin{equation}
    \label{eq:non-equilibrium-SA}
P = \frac{1}{1 + e^{\sum_i \Delta E_i/ T_i}},
\end{equation}
where $\Delta E_i$ is the energy change of the $i$-th term in the Hamiltonian and $T_i$ is the temperature of the $i$-th term.
It is easily to verify that this acceptance probability returns to the Glauber dynamics when $T_i = T$ for all $i$.
In \Cref{proof-of_non-equilibrium-SA}, we show this non-equilibrilium SA is equivalent to an equilibrilium SA with a layer dependent energy scale.



\subsection{Error rate in one block}

We first test the hypothesis mentioned in block-wise annealing. It claims that the error rate 
should decrease exponentially with the time $t_{th}$ the Markov transition matrix is applied.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\columnwidth,keepaspectratio]{../notes/images/eps_vs_t_directannealing.pdf}
    \caption{The error rate $\epsilon$ does not decrease exponentially with the total annealing time $t_{th}$
    }
    \label{questionplot_directannealing}
\end{figure}

We set a beginning temperature of $T_{max} = 10$ and a final temperature of $T_{min} = 10^{-5}$.
There are $t_{th}$ flip step in total, the temperature at step $i$ is given by $T_i = T_{max} + (T_{min} - T_{max})\frac{i}{t_{th}}$, and we perform
a flip try to each spin respectively at each time step. The block size is set to be $N = M = 7$. 
Denote one sample is error if the energy of the final state after step $t_{th}$ is not the ground state energy.

Finally, we examine the error rate $\epsilon$ as a function of the total annealing time $t_{th}$.
However, as \Cref{questionplot_directannealing} shows, it at least does not decrease exponentially with $t_{th}$.

\subsection{Error rate in the moving heat bath}

We also expect the error rate would decrease exponentially with the annealing time while $N, M$ is fixed, i.e. 
\begin{equation}
    \ln(\frac{1}{\epsilon}) \propto t_{\rm total}
\end{equation}

However, we found that the error rate is only inversely proportional to the annealing time, i.e,
$\frac{1}{\epsilon} \propto t_{\rm total}$. 

\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth,keepaspectratio]{../notes/images/eps_vs_t.pdf}
    \caption{(a): The error rate $\epsilon$ scales inversely with the total annealing time $t_{\rm total}$, which is different from the expected exponential scaling.
    (b): The log plot implies that $\ln(v)$ is linear to $\ln(M)$, which then implies $v \propto (\frac{1}{M})^{b}$, where $b$
    is the slope of the line.}
    \label{questionplot}
\end{figure}

\Cref{questionplot}(b) shows that the moving speed $v$ is at least proportional to $\frac{1}{M}$,
 which means we need at least $O(M^2)$ time to achieve a fixed error rate. This is correspond to the relationship
 between $\epsilon$ and $t_{\rm total}$ when $N, M$ is fixed.

% The main problem is that $\epsilon$ does not obey the hypothesis that it decreases exponentially with the annealing time.
% We come up with this hypothesis based on the intuition that the sampling procedure is a Markov chain, 
% the proportion of the non-steady-state distribution should decrease exponentially with the number of samples. 

% \ym{I think this hypothesis make sense, but how to check whether the code is correct or not?
% I set a minimum temperature of $10^{-5}$ in simulation. Should this be the reason?}

This problem has nothing to do with the temperature distribution model. Same relationship between
$v$ and $M$ also exist when we choose to anneal this system with a gaussian type temperature distribution.

\subsection{Main Result}
Here we conducted numerical experiments to examine how the cooling time $T_{c}$ ($N \times t_{\rm total}$) scales with (i)
the ``Space'' dimension $N$, (ii) the ``Time'' dimension $M$, (iii) the ``Width'' of the heat zone $W$, and (iv) the temperature gradient $\Lambda$.
% \ym{I have not conducted experiments for (iii) and (iv).}
If a SA run finds the ground state, we mark it as a success, otherwise, it is a failure.
The success probability is directly related to the moving speed $v$ of the heat bath.
The slower the heat bath moves, the higher the success probability.
We use the number of SA sweeps to reach $50\%$ success probability as the metric to measure the cooling time, which could be obtained by tuning the speed $v$ through binary search.

\begin{figure*}[]
    \centering
    \vspace*{-\topskip}
    \includegraphics[width=\textwidth,keepaspectratio]{../notes/images/main_result.pdf}
    \caption{Simulated annealing results for the toy model in \Cref{eq:toy-hamiltonian}.
    (a) The log plot implies that $\ln(v)$ is linear to $\ln(M)$, which then implies $v \propto (\frac{1}{M})^{b}$, where $b$
    is the slope of the line. (b) The total annealing time $T_c$ scales linearly with the
    number of spins in single layer $N$. (c) The total annealing time $T_c$ ($N \times t_{\rm total}$) with different annealing methods,
    including heat bath (HB) moving forward, heat bath (HB) moving backward, Global annealing (GA) with input spins fixed and
    Global annealing (GA) with output spins fixed. The latter three methods are all hard to cool the system.
    }
    \label{fig:main-numerical-result}
\end{figure*}

% \jinguo{you need to explain the results... what can we learn from the results?}
\Cref{fig:main-numerical-result} (a) shows a polynomial correlation between $\ln(M)$ and $\ln(v)$, which then
implies $v \propto (\frac{1}{M})^{b}$, where $b$ is the slope of the line. It then comes that $T_{c} = \frac{M}{v} \propto M^2$, which does not match our theory.
\Cref{fig:main-numerical-result} (b) shows a linear relationship between $T_c$ and $N$.
Combining the above two results, we can conclude that, from numerical sight, the time complexity should at least scales as $O(NM^2)$.
...
\Cref{fig:main-numerical-result}(c) compares different annealing methods for computing from the deterministic direction and the non-deterministic direction.
The computing time scales exponentially with the number of layers $M$ for all non-deterministic methods, regardless of the direction of computation.
It is consistent with the computational complexity of the circuit satisfiability problem.
For the deterministic computation, the global annealing method also takes exponential time, which is consistent with our theoretical prediction in \Cref{hyp:overlap-gap}.
The heat bath moving forward method is the most efficient.
% \jinguo{we need to add this plot back, how about using linear axis in figure (c)?}
...
\begin{figure*}[]
    ...
    \includegraphics[width=\textwidth,keepaspectratio]{../notes/images/main_result.pdf}
    \caption{Simulated annealing results for the toy model in \Cref{eq:toy-hamiltonian}.
    (a) The moving speed $v$ of the heat bath is proportional
    to $\ln(M)^{f(\Lambda)}$, where the power function $f(\Lambda)$ can be determined
    by fitting the slope. (b) The total annealing time $T_c$ scales linearly with the
    number of spins in single layer $N$. (c) The total annealing time $T_c$ ($N \times t_{\rm total}$) with different annealing methods,
    including heat bath (HB) moving forward, heat bath (HB) moving backward, Global annealing (GA) with input spins fixed and
    Global annealing (GA) with output spins fixed. The latter three methods are all hard to cool the system.
    \jinguo{Is it possible to verify that the Ising model is similar to the toy model?}
    \ym{If a comparison between Ising model and toy model is a must, plese inform me.}
    }
    \label{fig:main-numerical-result}
\end{figure*}

% \jinguo{you need to explain the results... what can we learn from the results?}
\Cref{fig:main-numerical-result} (a) shows a polynomial correlation between $\ln(M)$ and $v$. By fitting the curve, we can determine the constant $c$ in \Cref{eq:moving-speed}.
An empirical value of $c \approx 1/3$ is obtained.
\Cref{fig:main-numerical-result} (b) shows a linear relationship between $T_c$ and $N$.
Since in a real TC, a layer is cooled in parallel, the true computing time $t_{\rm total} \sim T_c / N$, which cancels the linear scaling of $N$.
We conclude that the real computing time does not scale with the computer memory size $N$.
%we can conclude that the time complexity should scales as $NM\log(M)^{1+\frac{1}{3\ln(\Lambda)}}$, 
%which is consistent with our theoretical prediction in \Cref{eq:total-annealing-time}.
...
It is consistent with the computational complexity of the circuit satisfiability problem.
For the deterministic computation, the global annealing method also takes exponential time, which is consistent with our theoretical prediction in \Cref{hyp:overlap-gap}.
The heat bath moving forward method is the most efficient, the computing time is almost perfectly linear to $M\ln(M)^{(1+\frac{c}{\ln(\Lambda)})}$.\jinguo{we need to add this plot back, how about using linear axis in figure (c)?}
...
Ising model, we used Linear Programming algorithm.
...
Here with a Ising model consists $N$ atoms, we choose $n$ to be $\frac{N(N-1)}{2} + N$, which means each variable $x_i$ represents a correspond interaction energy $J_{u, v}$ or a correspond onsite energy $h_u$, we assigned aliases to these $x_i$ as $x_{u,v}$ or $x_{u}$.
...
We set the $S_{\text{min}}$ to be the eight degenerate ground states of the Ising model to implement the $110$ rule.
...
\hline
\end{tabular}
\caption{The eight degenerate ground states of the Ising model to implement the $110$ rule.}
\end{table}
...
\section{Simulated annealing algorithm}\label{app:sa}
...
Therefore, based on HeatBath acceptance rule, we made a slight modification to the classical SA model.\jinguo{??? shouldn't it be Glauber dynamics?}
...
dynamics of this Ising model. However, result shows that it is
extremely hard to find the solution when the number of layers exceed
$4$ (input layer is pinned).

The reason maybe that the mapped Ising model lies in the hard
...
random Ising model, there exist hard instance that can't be solve.
