...
\jinguo{Please shorten this section!}
...
\subsubsection{A more realistic heat bath}\label{estimation-of-the-computing-time}
...
When one spin is flipped in the SA, there would be some energy difference
$\Delta E_{i}$ in field $i$ with temperature $T_i$. 
We choose the probability to accept this flip to be
...
heat-equilibrium, the probability reverts to the Glauber dynamics.
We also provided a more general proof by constructing another equivalent model in \Cref{proof-of_non-equilibrium-SA}.
...
\begin{figure*}[]
    ...
    \caption{Numerical Result. (a) The moving speed $v$ of the heat bath is proportional
    ...
    number of spins in single layer $N$. (c) The total annealing time $T_c$ with different annealing methods,
    ...
    \jinguo{(1) We should make the lines in (c) fit better with the data points. (2) Please complete the caption. You need to at least explain the short notations: GA, HB. (3) Please use log-scale axes for (a) and (c), instead of taking log directly on the data points. $\ln(\ln(M))$ is difficult to interpret.}}
...
empirical constant $c$ in \Cref{eq:total-annealing-time} to be $3$. 

\Cref{fig:main-numerical-result}(b) shows 
the the relationship between $T_c$ and $N$ should be linear. Combining these two results, 
...
\Cref{fig:main-numerical-result}(c) compares the time complexity with different annealing
methods. It costs exponential time to cool the system with the global annealing method no matter
we set the system to perform a deterministic or non-deterministic computation. Using the blocking
annealing method to perform a non-deterministic computation is still hard, too.
