...
It can be rigorous proved that the cooling process is hard if the energy landscape of the optimization problem exhibits the overlap gap property~\cite{Gamarnik2021}. If a problem has the overlap gap property, then it can not be solved by any local algorithms in time polynomial to the system size, including both the physical annealing and the simulated annealing method used in this work.
...
Since there is no standard approach to model the physical annealing process, we consider a theoretical model for cooling, which is the simulated annealing (SA)~\cite{Kirkpatrick1983} algorithm that detailed in \Cref{app:sa}.
...
These blocks are initialized to a low enough temperature, so that the spins freeze into a random local minimum.
Then we anneal these blocks sequentially from left to right by heating up a block to a temperature higher than the energy scale and then cooling it down.
...
Given a slow enough cooling rate, the spins in the block will settle into the global ground state.
...
Instead of block-wise annealing, we propose to move the heat bath from the left surface to the right surface at a constant speed $v$.
...
\subsubsection{Question about the above theory}

\ym{Please check this part}

We expect the error rate would decrease exponentially with the annealing time while $N, M$ is fixed, i.e. 
\begin{equation}
