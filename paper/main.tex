...
\begin{equation}
    ...
    \Delta \sim e^{-W}.
\end{equation}
Consequently, $t_{th}$ is related to $\epsilon$ by $\left( 1-\Delta \right) ^ {t_{th}} = \epsilon$, which gives $t_{th} \sim \ln(\frac{M}{W})e^{W}$, and the total sweep times scale as $\frac{M}{W}\ln(\frac{M}{W})e^W$.
...
The total sweep time with a fixed inputs size $N$ can then be calculated as 
