...
In this article, We develop a new type of TC shown in \Cref{Surface-programmable-thermodynamic computer}(a), which features a crystalline structure based on a universal elementary CA, makes it easier to be constructed in real life. 
...
Directly synthesizing the logic circuit for the universal $110$ rule (\Cref{eq:110rule}) requires at least 3 ancilla spins, which is far from optimal. By converting the gadget finding problem to a linear programming problem~\cite{}, we propose a provable optimal method to encode any CA rule into the Ising model, which is described in \Cref{sec:gadget-design}.
...
Its ground state with energy $0$ encodes a $110$ CA, where the direction of computation is from the left to right.
\subsection{Numerical result}


\ym{I will move this figure to AppendixB to show the equivalent 
between heat bath and energy gradient. Another plot would be put here that
show the TL v.s. $m\ln^{1+\frac{1}{c\ln(\Lambda)}}m$}
