...
In this work, we focus on the energy based models, which are one of the most powerful models for computation.
For example, the Ising model can formulate any problems in NP~\cite{gu2012encoding, lucas2014ising,mohseni2022ising}.
However, Ising models with random coupling are notoriously hard to thermalize, even with the help of quantum entanglement~\cite{boixo2013experimental, boixo2014evidence, Pichler2018, Ebadi2022, Nguyen2023}.
%There is no evidence yet that they can significantly outperform existing algorithms run on classical computers~\cite{}.

Can we utilize a natural energy based model for efficient computation?
Intrigued by this question, we designed an energy based TC as a translational invariant Ising model with universal computation capability on its surface, and study how to efficiently thermalize the system to perform computation.
Our TC implements a universal elementary CA, the translational invariance in both time and space dimension mimics a crystal in nature, which also eases the study of the scaling behaviors the thermalization process.

In the rest of this paper, we first show the design of crystalline structure that can be programmed on its surfaces to perform universal computation in \Cref{sec:Overview-of-main-results}.
The method we use to study the thermalization process is introduced in \Cref{sec:model-method}.
Finally, we reported numerical simulation results in \Cref{sec:numerical-result}. We show that even when the encoded problem is a deterministic computation, the system is not guaranteed to thermalize in time polynomial to the size of the crystal.
