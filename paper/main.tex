...
To overcome the overlap gap property, we propose to do block annealing, by introducing a heat zone with width $W$.
...
The system is devided into $\lceil\frac{M}{W}\rceil$ blocks,
where block $j$ consists of layers in the range $[(j-1)W+1, jW]$.
The system can then be cooled by sequentially cooling the blocks from left to right with an heat bath, where the temperature of the heat bath is gradually decreased.
...
Now we turn to determining the sweep time $t_{th}$ required to achieve $\epsilon$ error in each procedure. 
Let $\lambda_1$ and $\lambda_2$ be the largest and second largest eigenvalues of the Markov transition matrix, respectively. 
We assume the relationship between $\frac{\lambda_2}{\lambda_1}$ and $W$ can be expressed as
\begin{equation}
    \label{eq:spectral-gap}
    1 - \frac{\lambda_2}{\lambda_1} \sim e^{-W}.
\end{equation}
Consequently, $t_{th}$ is related to $\epsilon$ by:
\begin{equation}
    \left( \frac{\lambda_2}{\lambda_1} \right) ^ {t_{th}} = \epsilon \rightarrow t_{th} \sim \ln(\frac{M}{W})e^{W},
\end{equation}
and the total sweep times scale as $\frac{M}{W}\ln(\frac{M}{W})e^W$.
...
The effective width $W'$ then should scale as $\exp(-\Delta E_{max} \Lambda^{cW'(\Lambda)}) \sim \epsilon$, where $c$ is an empirical constant and can be determined during the experiment. 
...
\begin{equation}
    ...
    t_{total} \propto \frac{M}{W'(\Lambda)}(\ln(\frac{M}{W'(\Lambda)}))^{1+ \frac{1}{c\ln(\Lambda)}}
\end{equation}
